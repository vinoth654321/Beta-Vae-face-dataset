...
def split_dataset(data_partition):

    train_im_ids = []
    test_im_ids = []

    with open(data_partition, newline='') as csvfile:
...
def get_attributes():
    ...
    id_attr = {}
    with open('/home/mophy/DataSet/bate_vae/data/list_attr_celeba.csv', newline='') as csvfile:
        reader = csv.reader(csvfile, delimiter=',')
        attr = next(reader)[1:]
...
class ImageDiskLoader(torch.utils.data.Dataset):
    ...
    def __init__(self, im_ids,IMAGE_PATH):
    ...
    def __getitem__(self, idx):
        im_path = self.IMAGE_PATH + self.im_ids[idx]
        im = Image.open(im_path)
        im = im.convert('RGB')
        im = crop(im, 30, 0, 178, 178)
...
class ImageMemoryLoader(torch.utils.data.Dataset):
    ...
    def get_im(self, im_id, IMAGE_PATH):
        im_path = IMAGE_PATH + im_id
        im = Image.open(im_path)
        im = crop(im, 30, 0, 178, 178)
